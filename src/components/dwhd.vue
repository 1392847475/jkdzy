...
export default {
  ...
  mounted(){
    ...
    var url = base_URL + "/pub/api/guest/zjquestions";
    let that = this;
  ...
  var url = base_URL + "/pub/api/guest/zjquestions";
     axios.post(url,`wechatid=${sessionStorage.getItem('skopenid')}&pageNumber=${that.i}&pageSize=10`)
      ...
      //删除邀请
       sc(id){
          var base_URL = process.env.API_URL //获取环境变量配置
    var url = base_URL + "/pub/api/guest/refusedanswer";
        MessageBox({
      showCancelButton:true
    })
    MessageBox.confirm('是否拒绝回答','删除问题',).then(action => {
    if(action == 'confirm'){
      //console.log("确认");
